...
export function Terminal({ output, isLoading }: TerminalProps) {
  ...
  return (
    <div
      className="terminal-output card border border-gray-800 p-6 min-h-96 overflow-y-auto font-mono text-sm"
      role="log"
      aria-live="polite"
    >
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-800">
        <div className="flex items-center">
          <span className="text-blue-400 mr-3 font-bold" aria-hidden="true">
            $
          </span>
          <span className="text-blue-400 font-semibold tracking-wide">
            BuildProof Security Verifier
          </span>
      ...
      <div className="space-y-1.5" role="log">
        {displayedOutput
          ...
          .map((line, index) => (
            <div
              key={index}
              className="flex items-start gap-3 py-1.5 hover:bg-gray-900/20 transition-colors px-2 -mx-2"
            >
              <span
                className={`${getLineColor(line)} text-sm mt-0.5 font-bold w-4 flex-shrink-0`}
                aria-hidden="true"
              >
                {getLineIcon(line)}
              </span>
